 ...
 *
 * Invariants:
 *   1. `mcp.template.json` declares ONLY the `danx_dashboard` MCP server (chat
 *      is conversation + DB-backed card reads/edits — no playwright,
 *      no legacy danx-issue, no schema).
...
describe("issue-chat workspace shape (DX-351)", () => {
  it("mcp.template.json declares ONLY the `danx_dashboard` MCP server", () => {
    const path = resolve(HERE, "mcp.template.json");
    const content = JSON.parse(readFileSync(path, "utf-8")) as {
      mcpServers: Record<string, unknown>;
    };
    expect(content.mcpServers).toBeDefined();
    // Chat workspace = conversation + normalized dashboard DB views
    // (danx_dashboard). No playwright, no legacy danx-issue, no schema
    // MCPs.
    expect(Object.keys(content.mcpServers)).toEqual(["danx_dashboard"]);
  });

  it("mcp.template.json does NOT reference legacy schema placeholders", () => {
    const raw = readFileSync(resolve(HERE, "mcp.template.json"), "utf-8");
  ...
  it("workspace.yml required-placeholders match the resolver contract", () => {
    ...
    expect([...required].sort()).toEqual(
      [
        ...
        "DANX_REPO_NAME",
        // DX-1171 — board-only danx-dashboard-mcp qualified-board-id slug half.
        "DANXBOT_BOARD_NAME",
      ].sort(),
    );
