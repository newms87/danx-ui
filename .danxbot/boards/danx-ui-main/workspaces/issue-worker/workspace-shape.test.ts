 ...
 *      `mcp.template.json` MUST NOT declare a legacy `danx-issue`
 *      MCP server, MUST keep `playwright` + `danx_dashboard`, and
 *      `workspace.yml` MUST pin the placeholder contract the overlay
 *      injects at dispatch time (`DANX_REPO_ROOT` is the agent bash
 *      repo root, DX-660).
 *
...
describe("issue-worker workspace shape (Phase 4 invariants)", () => {
  it("mcp.template.json has no legacy `danx-issue` server entry and keeps `playwright` + `danx_dashboard`", () => {
    const path = resolve(HERE, "mcp.template.json");
    const content = JSON.parse(readFileSync(path, "utf-8")) as {
      mcpServers: Record<string, unknown>;
    };
    expect(content.mcpServers).toBeDefined();
    expect(Object.keys(content.mcpServers)).not.toContain("danx-issue");
    expect(Object.keys(content.mcpServers)).toContain("playwright");
    expect(Object.keys(content.mcpServers)).toContain("danx_dashboard");
  });

  // Pin the exact required + optional placeholder sets so a future
  // agent can't silently re-introduce tracker creds.
  // `DANX_REPO_ROOT` is the agent bash repo root (DX-660).
  it("workspace.yml required-placeholders + optional-placeholders match the resolver's contract", () => {
    ...
    expect([...required].sort()).toEqual(
      [
        ...
        "DANXBOT_WORKER_PORT",
        "DANX_REPO_ROOT",
        // `danx_dashboard` MCP server's env block (DX-704 Phase 2 wiring).
        // Overlay always injects all four (`src/dispatch/core.ts`).
        "DANXBOT_DASHBOARD_URL",
        "DANXBOT_DISPATCH_TOKEN",
        "DANX_REPO_NAME",
        // DX-1171 — board-only danx-dashboard-mcp's qualified-board-id slug half.
        "DANXBOT_BOARD_NAME",
        // `playwright` MCP server's `PLAYWRIGHT_URL` — the direct-to-container
        // playwright URL auto-injected by `src/dispatch/core.ts` on every
        // dispatch.
...
describe("issue-worker workspace env propagation (DX-660 integration)", () => {
  ...
  function setupRepoWithIssueWorkerSource(): {
    ...
    mkdirSync(resolve(repoDir, ".danxbot", "boards", "default", "workspaces"), {
      recursive: true,
    });
    cpSync(HERE, workspaceDir, { recursive: true });
    // Seed per-board settings to pass the issuePoller gate (env default
    // is false; operators opt in via this override).
    writeFileSync(
      resolve(repoDir, ".danxbot", "boards", "default", "settings.json"),
      JSON.stringify({
  ...
  function baseOverlay(workerPort: number): Record<string, string> {
    return {
      ...
      // `danx_dashboard` MCP server overlay — auto-injected by
      // `src/dispatch/core.ts` for every dispatch (DX-704 Phase 2).
      DANXBOT_DASHBOARD_URL: "http://localhost:5555",
      DANXBOT_DISPATCH_TOKEN: "test-dispatch-token",
      DANX_REPO_NAME: "test-repo",
      // DX-1171 — board-only danx-dashboard-mcp composes the qualified
      // board id `${DANX_REPO_NAME}:${DANXBOT_BOARD_NAME}`; auto-injected
      // by `src/dispatch/core.ts` (board.slug) for every dispatch.
      DANXBOT_BOARD_NAME: "default",
      // `playwright` MCP server's `PLAYWRIGHT_URL` — the direct-to-container
      // playwright URL auto-injected by `src/dispatch/core.ts` for every
      // dispatch (host mode → loopback host port, default 3283).
      DANXBOT_PLAYWRIGHT_URL: "http://localhost:3283",
      // Runtime mode — auto-injected by `src/dispatch/core.ts` (SG-349/eeb2186a).
      // Used by the playwright MCP server to decide loopback rewriting.
      DANX_RUNTIME: "host",
    };
  }
