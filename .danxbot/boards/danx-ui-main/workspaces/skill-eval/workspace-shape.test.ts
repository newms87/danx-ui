 ...
 *
 *   1. `mcp.template.json` matches `issue-worker`'s server set AND the per-server
...
describe("skill-eval workspace shape", () => {
  it("`mcp.template.json` mirrors issue-worker — same server set AND per-server shape", () => {
    const here = JSON.parse(
      readFileSync(resolve(HERE, "mcp.template.json"), "utf-8"),
    ) as { mcpServers: Record<string, Record<string, unknown>> };
    const issueWorker = JSON.parse(
      readFileSync(resolve(ISSUE_WORKER, "mcp.template.json"), "utf-8"),
    ) as { mcpServers: Record<string, Record<string, unknown>> };
  ...
  it("workspace.yml declares the required placeholders for dispatch core", () => {
    ...
    expect([...required].sort()).toEqual(
      [
        ...
        "DANXBOT_WORKER_PORT",
        "DANX_REPO_ROOT",
        // `danx_dashboard` MCP server's env (DX-704 Phase 2).
        "DANXBOT_DASHBOARD_URL",
        "DANXBOT_DISPATCH_TOKEN",
        "DANX_REPO_NAME",
        // DX-1171 — board-only danx-dashboard-mcp qualified-board-id slug half.
        "DANXBOT_BOARD_NAME",
        // `playwright` MCP server's direct-to-container URL, auto-injected by
        // `src/dispatch/core.ts` on every dispatch.
