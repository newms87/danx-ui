...
export function useCodeBlockManager(options: UseCodeBlockManagerOptions): UseCodeBlockManagerReturn {
	const { contentRef, codeBlocks, updateCodeBlockContent, updateCodeBlockLanguage, onCodeBlockExit, onCodeBlockDelete, onCodeBlockMounted, configureApp } = options;
	...
	function mountCodeViewer(wrapper: HTMLElement): void {
		...
		const app = createApp({
			setup() {
				...
				const computedFormat = computed(() => mapLanguageToFormat(reactiveLanguage.value) as import("../code-viewer/types").CodeFormat);
