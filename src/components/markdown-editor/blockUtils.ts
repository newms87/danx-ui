...
export function getTargetBlock(
	...
	if (includeCodeBlocks && (currentBlock.tagName === "PRE" || currentBlock.hasAttribute("data-code-block-id"))) {
		return currentBlock;
	}
	...
	while (current && current.parentElement !== contentRef.value) {
		...
		if (includeCodeBlocks && (current.tagName === "PRE" || current.tagName === "LI" || current.hasAttribute("data-code-block-id"))) {
			return current;
		}
	...
	if (current) {
		if (isConvertibleBlock(current)) return current;
		if (includeCodeBlocks && (current.tagName === "PRE" || current.hasAttribute("data-code-block-id"))) {
			return current;
		}
	}
 ...
 */
export function findLinkAncestor(node: Node | null, container: HTMLElement): HTMLAnchorElement | null {
	if (!node) return null;
